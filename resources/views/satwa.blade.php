...
<div class="d-flex justify-content-between align-items-center mb-5">
    ...
    <div class="btn-group shadow-sm filter-habitat">
        <a href="/satwa" class="btn {{ !request('habitat') ? 'btn-active' : 'btn-outline-hutan' }}">Semua</a>
        <a href="/satwa?habitat=darat" class="btn {{ request('habitat') == 'darat' ? 'btn-active' : 'btn-outline-hutan' }}">Darat</a>
        <a href="/satwa?habitat=laut" class="btn {{ request('habitat') == 'laut' ? 'btn-active' : 'btn-outline-hutan' }}">Laut</a>
        <a href="/satwa?habitat=udara" class="btn {{ request('habitat') == 'udara' ? 'btn-active' : 'btn-outline-hutan' }}">Udara</a>
    </div>
</div>

<div class="row g-4"> 
    @forelse($dataSatwa as $animal)
    <div class="col-md-4 col-sm-6">
        <div class="card h-100 animal-card shadow-lg">
            <div class="position-relative overflow-hidden" style="height: 220px;">
                <img src="{{ asset('img/' . $animal->gambar) }}" class="w-100 h-100 object-cover" alt="{{ $animal->nama }}">
                
                <!-- Overlay LIHAT DETAIL Berwarna Oranye -->
                <div class="card-hover-overlay d-flex align-items-center justify-content-center">
                    <a href="/satwa/{{ $animal->id }}" class="btn btn-detail shadow-sm">
            ...
            <div class="card-body text-center p-4">
                <h5 class="card-title mb-1">{{ $animal->nama }}</h5>
                <div class="habitat-label">
                    <i class="bi bi-geo-alt-fill me-1"></i> Satwa {{ $animal->habitat }}
                </div>
            </div>
        </div>
    </div>
